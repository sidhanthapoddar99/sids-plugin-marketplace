...
|---|---|---|
| [`project-setup`](plugins/project-setup) | How a repo is shaped: bootstrap, audit, or a single "where does this go" question. One tree, one `ctl` entrypoint, one origin, three env files. Fifteen reference pages and a copyable template with a working `ctl`: dev, up with configs, modifiers and presets, db migrate, check, gate. | Released (v0.8.0) |
