...
This lets the engine be consumed headlessly (server, CLI, a non-React host) while the UI layer stays optional. The **packaging** side of this split — the react-less `package.json` that mechanically enforces the boundary, and bundling `core` into one published artifact (`noExternal`) — is owned by `references/2-repo/01-layouts/06_embeddable-package.md` § single-artifact delivery.
...
(Repo-shape + packaging anti-patterns — treating the reference host as the product, peerDependency mistakes — are owned by `references/2-repo/01-layouts/06_embeddable-package.md`.)

## See also

- `references/2-repo/01-layouts/06_embeddable-package.md` — the repo shape, tree, publishing mechanics, single-artifact delivery, and `ctl` shape.
...
- `references/3-app/05-package/01_tokens-setup.md` — the `theme` seam maps onto the token system when the host passes token overrides.
- `references/2-repo/03-env-config/02_frontend-env-isolation.md` — for the reference host's own env, the usual `VITE_*` rules still apply.
