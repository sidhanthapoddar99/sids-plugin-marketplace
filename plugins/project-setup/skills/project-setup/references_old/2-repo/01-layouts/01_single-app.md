...
│   ├── src/<package>/               # ← src-layout (distributable) — internals owned by references/3-app/02-backend/00_app-skeleton.md
│   ├── tests/
│   ├── Dockerfile                   # optional
...
│   ├── config.yaml
│   ├── app/                         # ← flat run-service — internals owned by references/3-app/02-backend/00_app-skeleton.md
│   │   ├── main.py
│   │   └── …
...
Which shape this repo's service uses — flat `app/` for a run-service (launched, never built into a wheel) vs `src/`-layout for a distributable package/CLI — is an L3 app-skeleton decision. Pick by what the thing *is*, not by habit. Owned by `references/3-app/02-backend/00_app-skeleton.md`; don't restate the rationale here.
...
Move to Layout 02 (`references/2-repo/01-layouts/02_multi-app-monorepo.md`) when a second runnable thing appears — a frontend (even a small admin dashboard), a second backend, or a database this repo manages: introduce `apps/` then and move both under it. Escalation rules table: `references/02_decision-tree.md`.

## Anti-patterns

- **Loose code in the repo root** — putting `main.py`, `app/`, or `src/` directly in the repo root "because it's just one app." Keep the root clean; the code goes in a top-level service folder (`./<name>/`). Running `npm init` / `uv init` at the repo root has the same effect: the manifest, `node_modules/`, and run scripts take over the root. The only sanctioned root-manifest cases (editor extensions, a pure OSS package repo) are a *recorded* exception — see `references/2-repo/02-root-hygiene/00_root-and-hygiene.md`.
- **Nesting a single service under `apps/<name>/`** — `apps/` is for 2+ services; for one, use top-level `./<name>/`.
- **Wrong inner shape** — using `src/` for a run-service backend, or flat `app/` for a distributable. The run-service-vs-src-layout rule is owned by `references/3-app/02-backend/00_app-skeleton.md`.
...
- `references/2-repo/01-layouts/02_multi-app-monorepo.md` — 2+ apps (step up)
- `references/2-repo/02-root-hygiene/00_root-and-hygiene.md` — root contract, single-package containment, gitignore
- `references/3-app/02-backend/00_app-skeleton.md` — flat `app/` vs `src/`-layout, pyproject + uv flow, the skeleton
- `references/2-repo/06-runtime-environment/00_runtime-triad.md` — the runtime triad (`ctl`/docker/mise)
