...
Storage that survives instance termination: an object-store bucket (S3 / GCS / Azure Blob) mounted via s3fs / rclone, synced with `aws s3 sync` in the run wrapper, or a cloud volume the wrapper re-attaches on every retry. The path inside the box stays stable (`/checkpoints`); the `scripts/cloud/` wrapper owns the mount (`references/2-repo/07-ml-orchestration/00_custom-orchestrator.md`).
