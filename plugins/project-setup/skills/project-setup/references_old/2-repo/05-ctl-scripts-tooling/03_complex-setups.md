...
1. The `ctl` shell script grows past **~150 lines** of non-trivial logic (excluding comments/colour helpers) — this is **tripwire T7** (master table: `references/00_altitude-model.md`); crossing it obligates the binary escalation or a recorded deferral.
...
- The canonical Layout 05 shape: `docker/{singlenode,multinode,prod}/`, a Go orchestrator package, its binary at root. Cite a registered example from `references/handoffs/examples-registry.md` if one exists.
