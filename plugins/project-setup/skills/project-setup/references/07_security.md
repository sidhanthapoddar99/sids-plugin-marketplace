...
| Rule | Detail |
|---|---|
| Keys are backend-only | `.env`, read by the one loader. A key in `VITE_*` / `NEXT_PUBLIC_*` or shipped in a mobile or desktop build is published; rotate it. |
