...
App CI/CD (`references/repo-setup/tooling/ci-cd-future.md`) is about lint + test + build images. **ML CI/CD** has additional concerns: smoke-training, eval against benchmark, model registry / promotion, dataset versioning, and reproducibility.
...
| **Medium** — on main / nightly | eval against held-out set + small benchmark | 30–60 min on GPU (could be CI-managed or dstack-spawned) |
| **Expensive** — on tag / release | full training + full eval + model promotion | hours–days; dstack/sky managed |
...
## Medium tier — eval pass, spawn dstack run
...
jobs:
  eval:
    ...
    steps:
      - uses: actions/checkout@v4
      - name: Submit eval to dstack
        env:
          DSTACK_TOKEN: ${{ secrets.DSTACK_TOKEN }}
        run: |
          pip install dstack
          dstack apply -f tasks/eval.dstack.yml -y -d -n "eval-$(date +%Y%m%d)"
      - name: Wait + collect results
        run: |
          ./scripts/cloud/wait-for-run.sh "eval-$(date +%Y%m%d)"
          dstack logs "eval-$(date +%Y%m%d)" > eval-results.txt
      - name: Post results
        run: |
  ...
  train-and-publish:
    ...
    steps:
      ...
      - name: Submit full training
        run: |
          dstack apply -f tasks/train-release.dstack.yml -y -d -n "${{ github.ref_name }}"
...
```
scripts/cloud/
├── wait-for-run.sh             # polls dstack ps until run completes
...
└── ci-bootstrap.sh             # set up CI-side deps (dstack, aws cli, etc.)
```
...
run="$1"
while true; do
  status=$(dstack ps -f json | jq -r --arg name "$run" '.[] | select(.name==$name) | .status')
  case "$status" in
...
- `.github/workflows/eval.yml` (if dstack/SkyPilot in use)
