...
> Vite has no server runtime, so the proxy target lives in build/dev env that is effectively client-adjacent. With Next.js the equivalent proxy (`next.config` `rewrites()`) runs server-side and can use server-only env. See the Vite-vs-Next env-split comparison in `references/repo-setup/env-and-config/frontend-env-isolation.md`.
