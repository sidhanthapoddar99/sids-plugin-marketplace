...
- **Single replica** (dev, single-VM prod, SQLite / one Postgres): the backend **container's entrypoint runs `alembic upgrade head` on boot**, then launches the server. Simple, idempotent, no separate step. See `references/architecture/backend/alembic-default.md` § "Migrations in Docker".
...
- `references/repo-setup/docker/compose-as-deployment-modes.md` — the prod overlay
