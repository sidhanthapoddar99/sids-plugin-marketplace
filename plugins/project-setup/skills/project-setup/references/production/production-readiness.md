...
The "before you deploy" list. App-server worker config lives in `app-server-and-workers.md`; this is everything around it that separates a dev stack from a production-grade one.
...
Gunicorn handles 1–2 via `--graceful-timeout`. For app-level cleanup (closing pools), use lifespan hooks:
...
- **Match worker count to CPU limit** (see `app-server-and-workers.md`).
...
- **Single replica** (dev, single-VM prod, SQLite / one Postgres): the backend **container's entrypoint runs `alembic upgrade head` on boot**, then launches the server. Simple, idempotent, no separate step. See `references/python/alembic-default.md` § "Migrations in Docker".
- **Multiple replicas**: do **not** let every replica race to migrate on boot. Run migrations as a **separate one-shot step before** the app replicas start — a dedicated `migrate` service the app `depends_on`.

The one-shot service (multi-replica):
...
- `references/production/app-server-and-workers.md` — worker / recycling detail
- `references/env-and-config/secrets-matrix.md` — prod secrets handling
- `references/docker-compose/compose-as-deployment-modes.md` — the prod overlay
- `references/python/alembic-default.md` — migration mechanics
