...
How the backend actually runs in production. Dev uses a single hot-reload process (`uvicorn --reload`); production uses a process manager with multiple workers, recycling, and graceful shutdown. This is a decision the skill owns — it changes the Dockerfile `CMD`, the compose service, and resource sizing.
...
The prod overlay sets the worker env and resource limits:
...
- `references/production/production-readiness.md` — the broader pre-deploy checklist
- `references/docker-compose/compose-as-deployment-modes.md` — where the prod overlay lives
- `references/frontend/vite-proxy-nginx-pair.md` — nginx sits in front of these workers
