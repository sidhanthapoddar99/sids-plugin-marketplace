...
| `POSTGRES_PASSWORD` (CI tests) | Hardcoded `ci/ci` in workflow (it's ephemeral) |
| Production deploy creds | GitHub Actions secrets, scoped per repo |

## Vault — future state

When the project graduates from compose-on-VM:

- Vault Connect / 1Password Connect / cloud secrets manager
- CI fetches secrets via a short-lived token issued by an OIDC trust relationship
- No long-lived secrets in CI config

Document the migration path in `docs/secrets-matrix.md`; don't build it day one.
...
For now, the bootstrapper just creates `.github/workflows/` with a `.gitkeep` and prints a hint to read `references/repo-setup/tooling/ci-cd-future.md` for templates.
