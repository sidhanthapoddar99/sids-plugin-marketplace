...
The shipped template is **data-core-shaped**: it assumes a `postgres + redis` core, and several workers + the base compose default to it. A large, common class of projects has **no database** — static frontends, pure API gateways, stateless microservices, published SDKs, ML repos, this marketplace's own tooling. This doc is the topology analogue of `script-alternatives.md` (which covers *tool* swaps): what to change to express "I have no data layer."
