...
- **Docker structure** — profiled `compose.yaml` (data core + `[app]`/`[edge]`) plus at most one `--config=prod` and stackable `.m.` modifiers (`--expose`/`--traefik`). See `references/repo-setup/runtime/docker-overview.md`.
