...
└── my-product-deploy/              # ← the aggregator
    ...
    ├── ctl                         # the aggregator's own ctl (ctl up prod deploys)
    ├── scripts/
...
4. **`ctl up prod`** (the aggregator's own dispatcher) — pulls images, runs migrations (via a one-shot container), restarts services.
