...
Steps 2–4 are delegated to a process runner — **don't hand-roll PID juggling** (see `references/scripts/global-wrapper-dispatcher.md` § "Delegate"). Default `process-compose`; `mprocs` as a lighter option; a bash `trap` only as a 1–2-process fallback.
...
1. **Vite proxy** (default) — `vite.config.ts` proxies `/api/*` to the backend's host port, read from env. CORS-free; no extra container. See `references/frontend/vite-proxy-nginx-pair.md`.
...
- Different env vars for in-container vs on-host dev — drift; standardise on the env contract (`references/env-and-config/env-precedence.md`).
...
- `references/scripts/global-wrapper-dispatcher.md` — `ctl dev` / `ctl prod` / `ctl up`
- `references/scripts/three-startup-paths.md` — the three documented ways to start
- `references/frontend/vite-proxy-nginx-pair.md` — dev proxy → prod nginx
