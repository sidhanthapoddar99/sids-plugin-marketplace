...
For projects that have a mobile surface. The default here is **native** apps in this stack — not React Native, not Flutter.
...
- Per-developer signing certs without a shared secrets manager — onboarding suffers
- API base URL hard-coded — use build flavours (debug vs release) with different bases

## See also

- `references/2-repo/03-env-config/02_frontend-env-isolation.md` — build-time vs runtime (mobile is heavily build-time)
- `references/2-repo/05-ctl-scripts-tooling/06_ci-cd-future.md` — adapt for mobile-specific runners
- `references/handoffs/examples-registry.md` — cite a registered mobile repo if one exists
