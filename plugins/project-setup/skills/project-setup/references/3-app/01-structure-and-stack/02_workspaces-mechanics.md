...
The decisions these bodies serve live elsewhere: whether to introduce a workspace at all (2+ frontends that actually share code) is **owned by `references/2-repo/01-layouts/02_multi-app-monorepo.md` § "Scaling: more than one frontend"**; where the workspace roots (repo root for JS-only, frontend group folder for polyglot) and which scope each package sits at are **owned by `references/2-repo/01-layouts/00_grouping-topology.md`**. Package internals (export surface, what lives in each package, tailwind wiring, tripwire T4) are **owned by `references/3-app/05-package/00_shared-packages.md`**.

## Where the config files sit

The workspace root holds orchestration-only manifests — no runtime deps, no source (`references/2-repo/02-root-hygiene/00_root-and-hygiene.md`, tripwire T10):
...
└── packages/              # shared packages (internals → references/3-app/05-package/00_shared-packages.md)
```
...
Rooting choice (why the anchor moves) → `references/2-repo/01-layouts/00_grouping-topology.md`.
...
**Every `VITE_*` / `NEXT_PUBLIC_*` that affects the build must be listed in `globalEnv`.** Omitting one means turbo's cache key misses that variable — stale builds from a warm cache. Build-var isolation itself (which prefix leaks to the client) is owned by `references/2-repo/03-env-config/02_frontend-env-isolation.md`.
...
`ctl`'s broader shape (dev vs up axes, size tripwire T7) is owned by `references/2-repo/05-ctl-scripts-tooling/00_script-overview.md`.
...
- `references/2-repo/01-layouts/00_grouping-topology.md` — when to introduce a workspace, rooting, package placement (the decisions these bodies serve)
- `references/3-app/05-package/00_shared-packages.md` — what lives inside `packages/*`, export surface, tailwind wiring, T4
- `references/3-app/03-web-app/00_app-skeleton.md` — the app's own `src/` skeleton and workspace reconciliation
- `references/2-repo/02-root-hygiene/00_root-and-hygiene.md` — orchestration-only root manifest (T10)
