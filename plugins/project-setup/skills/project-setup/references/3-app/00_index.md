...
The internal structure of each runnable app and each workspace package: its skeleton, which stack it uses, how features group, where shared code and contracts live, how its data layer is owned and served, its AI and security surfaces, and how it packages itself — plus the non-web app kinds (mobile / desktop / PWA). Binds when the app is created — mostly derived from L2's decisions plus a few per-app questions — and is recorded in the CLAUDE.md structure block. Inherits its slot from L2; hands folder-level contracts down to L4. This is the **index of L3 decisions → their owner files**; it never restates a rule.
...
| **Stack choice** | Which language / framework / runtime an app uses, per kind, with criteria; firm defaults, deviate with a recorded reason. | `references/3-app/01-structure-and-stack/01_stack-decision.md` |
| **Workspace wiring (mechanics)** | pnpm-workspace globs, turbo pipeline, bun/catalog config bodies, `ctl` delegation — mechanics only; the topology decision is L2. | `references/3-app/01-structure-and-stack/02_workspaces-mechanics.md` |
...
| **Type / DTO ownership** | `models.py` = the feature's API contract; frontend types inferred from zod in `api/`; cross-app entities in `packages/types`; no cross-domain DTO imports; no shared models package between backends. | `references/4-feature/03_types-and-contracts.md` (housed at L4) |
...
| **Design tokens + theming** | One `tokens.css` in the styles package; `[data-theme]` light/dark switching; shadcn wired to tokens. Usage discipline is L4. | `references/3-app/05-package/01_tokens-setup.md` |
...
Each feature folder receives its shape contract (`{router,service,repository,models}.py` / the feature's frontend subdivision axes), the type-placement rules, and the tripwire numbers — via the CLAUDE.md structure block, since L4 agents may never load this skill. See `references/4-feature/00_index.md`.
...
- `references/2-repo/00_index.md` (step up) · `references/4-feature/00_index.md` (step down)
