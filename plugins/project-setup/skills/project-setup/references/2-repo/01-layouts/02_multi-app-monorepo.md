...
- Contrast: a single runnable app → **Layout 01** (`references/2-repo/01-layouts/01_single-app.md`). Apps that ship and release independently, or another repo that depends on this one → **Layout 03** (`references/2-repo/01-layouts/03_polyrepo-aggregator.md`).
...
│   │   ├── app/                    # ← FLAT run-service — skeleton owned by references/3-app/02-backend/00_app-skeleton.md
│   │   ├── tests/
│   │   ├── Dockerfile
...
│       ├── tailwind.config.ts
│       ├── tsconfig.json
│       ├── src/                    # ← src/ skeleton (styles/tokens.css, components, lib, pages) owned by references/3-app/03-web-app/00_app-skeleton.md + references/3-app/05-package/01_tokens-setup.md
│       ├── Dockerfile
...
If frontends don't actually share code, don't introduce workspaces — just use the canonical layout twice. Once you cross into needing them, **delegate the detail** to `references/3-app/01-structure-and-stack/02_workspaces-mechanics.md` (pnpm/turbo setup, `turbo.json` `globalEnv`, per-app env isolation) and `references/3-app/05-package/00_shared-packages.md` (the `packages/ui` + `packages/styles` tokens contract). Those refs are the source of truth; this section only tells you when to reach for them. Where the workspace and its packages *sit* is the grouping-topology decision — see `references/2-repo/01-layouts/00_grouping-topology.md`.
...
Either way the routing contract is unchanged — everything under `/api/*` behind the same proxy pair (`references/2-repo/04-docker/04_proxy-and-exposure.md`).
...
- **Two DDL owners** — more than one backend writing migrations against shared schema (owned by `references/3-app/02-backend/02_two-plane-split.md`).
...
- `references/2-repo/01-layouts/01_single-app.md` — one runnable app (step down)
- `references/2-repo/01-layouts/03_polyrepo-aggregator.md` — independent release cadences (step up)
- `references/2-repo/01-layouts/04_ml-project.md`, `.../05_infra-orchestrator.md`, `.../06_embeddable-package.md`
- `references/2-repo/01-layouts/00_grouping-topology.md` — flat / plane-grouped / hybrid, workspace rooting, package placement (T1)
- `references/2-repo/02-root-hygiene/00_root-and-hygiene.md` — root contract and gitignore
- `references/3-app/02-backend/02_two-plane-split.md` — separate admin/user backends + the `apps/db` migrations owner
- `references/3-app/01-structure-and-stack/02_workspaces-mechanics.md`, `references/3-app/05-package/00_shared-packages.md`
- `references/3-app/03-web-app/00_app-skeleton.md`, `references/3-app/02-backend/01_domain-grouping.md` — what's *inside* each app
- `references/2-repo/03-env-config/`, `references/2-repo/06-runtime-environment/00_runtime-triad.md` (the runtime-triad map — start here)
- `references/3-app/10-deployment/00_serving.md`, `references/2-repo/04-docker/05_production-readiness.md`
