...
3. **`.env.proxy` defines every piece.** One `<PIECE>_HOST/_PORT/_PREFIX` block each. The service binds `_PORT` and mounts `_PREFIX` from it; nginx and the dev proxy read the same keys to route; a frontend reads its `_PREFIX` as `base` / `basePath`. Definition and route are one key, so they cannot drift.
...
| Edge config | `apps/example-multi-web-app/nginx/nginx-dev.conf.template` | `apps/example-multi-web-app/nginx/nginx.conf.template` |
...
Adding a backend: one proxy entry in `vite.config.ts`, one `location` in both nginx templates, one `<X>_UPSTREAM` literal in compose, one `<X>_HOST/_PORT/_PREFIX` block in `.env.proxy.template`. Frontend code does not change.
...
| Prod | `ctl up +env_override`. The modifier sets `API_UPSTREAM: ${API_HOST}:${API_PORT}` on `web` and `ENGINE_URL: ${ENGINE_URL}` on `api`, from `.env.proxy`; `DATABASE_URL` from `.env.secrets`. `ctl` refuses the modifier when a mapped key is blank. |
...
2. Final stage `nginx:<version>`: `COPY` each output under its prefix in `/usr/share/nginx/html/`; `COPY nginx/nginx.conf.template` to `/etc/nginx/templates/`. nginx renders it at start from the container environment. Listens on 8080, unprivileged. `+expose_web` publishes `${HTTP_PORT}:8080`.
...
**Adding a static frontend:** a folder under `apps/example-multi-web-app/`, one build stage and one `COPY --from` in the Dockerfile, one `location` in both templates, one `<X>_PORT/_PREFIX` pair in `.env.proxy.template`, one line in `app_names` in `scripts/dev/dev.sh`.
