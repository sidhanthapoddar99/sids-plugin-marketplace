...
Run this in order before proposing any layout. The order is **level-ordered** (see `references/levels/00_altitude-model.md`): L1 ecosystem → L2 repo → L3 per-app. **L4 is never asked** — feature-level conventions are installed via the CLAUDE.md blocks, not decided in an interview.
...
4. **Docs: in-repo `docs/` or a separate docs repo?**
   - Default **in-repo** for a single-repo product; **separate `<product>-docs`** when the product spans multiple repos or docs release independently (`references/levels/01_ecosystem.md`). Never both.
...
6. **Does any surface need a separate identity/security plane** — operator/admin vs end-user (own credentials table, internal-only exposure, independent deploy cadence)?
   - Yes → the two-plane split: separate backends + the neutral `apps/db` migrations owner (`references/architecture/backend/two-plane-split.md`). The backend count falls out of this answer — don't guess it from Q5 alone.
...
9. **Grouping topology** (when 2+ apps): flat `apps/` (default), plane-grouped (`apps/server/` + `apps/client/`), or hybrid?
   - Apply the tripwire default — plane-grouped once 2+ frontends AND (2+ backends OR frontend-only packages) — but **ask when both fit**; record the pick. See `references/repo-setup/layouts/02_multi-app-monorepo.md` § grouping topology.
10. **What languages?**
...
13. **If workspace: which shared packages, at which scope, rooted where?**
    ...
    - **Scope**: frontend-only packages live inside the client group; cross-plane packages force root `packages/` (consumer rule — `references/architecture/frontend/multi-frontend-workspaces.md` § rooting).
    - **Rooting**: JS-only repo → workspace at repo root (orchestration-only manifest); polyglot → workspace at the frontend group folder (`references/repo-setup/root-and-hygiene.md`).
...
18. **`ctl` subcommands**: which day-to-day flows need shortcuts?
    ...
    - Install is **copy-verbatim from `assets/snippets/scripts/`**, then adapt by deletion — the conformance floor in `references/repo-setup/runtime/script-overview.md`.
...
21. **Production serving**: per language — Python: gunicorn + uvicorn workers (count, recycling, graceful timeout); Rust/Go/Node: replicas. Confirm health endpoints, graceful shutdown, limits, migrations-as-pre-traffic-step (`references/architecture/production/`).
...
23. **Open source vs private**: license, CI defaults. For a pure OSS **package** repo, this is also where the root-manifest exception may apply — record it if taken (`references/repo-setup/root-and-hygiene.md`).
...
27. **Databases — pick the right floor** (`references/architecture/database/choosing-a-database.md`): SQLite vs Postgres (concurrent writers, sharing, extensions); in-process memory vs Redis (coupled to worker count, Q21); Mongo/Neo4j/Kuzu/Seaweed per requirement.
...
30. **Docs handoff**: for in-repo docs, create the `docs/` slot and point at the docs plugin's init command (`references/integrations/docs-integration.md`).
...
39. **Training cadence**: one-shot / sweep / continuous / batch → checkpoints + retry vs managed service (`references/architecture/ml-orchestration/`).
...
42. **Agent access to the remote box?** → `agent-ssh-access.md`; permissions configured explicitly.
