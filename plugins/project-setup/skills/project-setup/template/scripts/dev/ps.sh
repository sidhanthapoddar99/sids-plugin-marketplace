...
#
#   dev      host processes on the dev ports (API_PORT / ENGINE_PORT / WEB_*_PORT / DASHBOARD_PORT)
...
#
#   ctl ps                      interactive browser: hover a process — Enter = action menu ·
#                               a = attach · k = kill · q = quit  (arrows move; j/k vim-nav
#                               is off here because k kills)
#   ctl ps --list               the containers table + port map only (also the no-TTY behaviour)
...
gather() {
  ...
  for spec in "api:${API_PORT:-8000}" "engine:${ENGINE_PORT:-8080}" "landing:${WEB_LANDING_PORT:-3001}" \
              "app:${WEB_APP_PORT:-5173}" "docs:${WEB_DOCS_PORT:-4321}" "dashboard:${DASHBOARD_PORT:-3000}"; do
    name="${spec%%:*}"; p="${spec#*:}"
...
do_kill() {  # $@ = requested ports (may be empty → interactive pick)
  ...
  while true; do
    ...
    action=()
    if (( cli )); then
      tui_select --into action --horizontal --header "Free these ports?" -- Free Cancel || { say "cancelled."; exit 0; }
    else
      tui_select --into action --horizontal --header "Free these ports?" -- Free Back Cancel || { say "cancelled."; exit 0; }
    fi
...
browse() {  # interactive: hover a process — Enter menu · a attach · k kill · q quit
  ...
  while true; do
    ...
    case "${TUI_SELECT_KEY:-}" in
      a) attach_entry "$cur" || true ;;
      k) printf '\n'; free_entry "$cur"
         say "reproduce:  ${C_DIM}ctl ps kill $(cut -d'|' -f2 <<<"$cur") -y${C_RESET}"; printf '\n' ;;
      *) printf '\n'
         action=()
         tui_select --into action --horizontal --header "$(entry_label "$cur")" -- Attach "Free port" Back || return 0
