...
usage() { print_help "test" "Run test suites across the apps." \
  'test [api|engine|web|site|cli|database|e2e] [-h]' \
"Arguments
...
case "$target" in
  all)      run_py apps/api; run_py apps/database/postgres; run_rs apps/engine; run_js apps/web; run_js apps/site; run_go apps/cli ;;
  api)      run_py apps/api ;;
  database) run_py apps/database/postgres ;;
  engine)   run_rs apps/engine ;;
  web)      run_js apps/web ;;
  site)     run_js apps/site ;;
  cli)      run_go apps/cli ;;
  e2e)      exec bash "$CTL_ROOT/scripts/test/e2e.sh" "${@:2}" ;;
  *)        die "unknown target: $target (all|api|engine|web|site|cli|database|e2e)" ;;
esac
