#!/usr/bin/env bash
# container/health.sh — `ctl health`. One-shot health table of the data core (or named services).
set -euo pipefail
...
usage() { print_help "health" "One-shot health table for the stack's containers." \
  'health [svc…] [-h]' \
"Arguments
  svc…            services to check (default: the data core — ${DATA_SVCS[*]:-none} — else every service)
