...
mkdir -p "$LOGS_DIR/perf"; stamp="$(date +%Y%m%d-%H%M%S)"
failed=() ran=0
while IFS= read -r d; do [[ -n $d && -d "$d/tests/perf" ]] || continue
  step "perf $d (pytest-benchmark)"; ran=$((ran+1))
  ( require_tools uv && cd "$d" && uv run pytest tests/perf --benchmark-only --benchmark-json="$LOGS_DIR/perf/$stamp-${d##*/}.json" ) || failed+=("$d")
done < <(gate_apps pyproject.toml)
while IFS= read -r d; do [[ -n $d && -d "$d/tests/perf" ]] || continue
  require_tools k6
  while IFS= read -r s; do [[ -n $s ]] || continue
    step "perf $d (k6 ${s##*/})"; ran=$((ran+1))
    k6 run --summary-export="$LOGS_DIR/perf/$stamp-${s##*/}.json" "$s" || failed+=("$s")
...
while IFS= read -r d; do [[ -n $d && -d "$d/benches" ]] || continue
  ...
  ( require_tools cargo && cd "$d" && cargo bench ) | tee "$LOGS_DIR/perf/$stamp-${d##*/}.txt" || failed+=("$d")
...
ok "gate perf green — $ran targets · results in $LOGS_DIR/perf/$stamp-*"
