...
| Gate ladder | `lint typecheck test check` `<+ dead · audit · build · e2e, as each is switched on>` (`clones`, `fuzz`, `perf` by name) |
...
- Feature code composes primitives. Raw utilities are for layout glue only. A look that does not exist yet is added to the primitive as a variant, so the next feature gets it for free.
