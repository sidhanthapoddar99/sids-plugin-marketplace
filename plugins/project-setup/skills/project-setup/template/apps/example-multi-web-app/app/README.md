...
Run from here: `ctl dev app` (exports the root .env file, then `bun dev`). Prefix: `WEB_APP_PREFIX` from `.env`. No `.env` here. Test: `bun test`. The browser suite (`e2e/`) is an add-on.
