...
# 1. find the repo root (walk up until a dir holds `ctl`); load_dotenv(ROOT/.env, override=False).
#    Under docker there is no .env file: compose set the environment, so this step is a no-op.
