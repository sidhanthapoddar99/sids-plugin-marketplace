...
| Browser suite | `apps/example-single-web-app-vite/e2e/` | One Playwright smoke spec. `ctl test e2e` runs it against a built stack on a throwaway `DATA_DIR`. | There is a user flow worth protecting. | Copy into the frontend that owns `/`, as `e2e/`. Add `e2e` to `RUNGS` in `scripts/gate/all.sh` and to the `Gate ladder` row in `AGENTS.md`. |
