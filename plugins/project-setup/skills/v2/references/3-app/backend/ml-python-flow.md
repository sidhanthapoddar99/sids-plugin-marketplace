...
No docker compose. No frontend. No `pyproject.toml` typically (but optional if you want to install `apps/<project>/` as a package into the env). Full ML repo shape: `references/2-repo/layouts/04_ml-project.md`.

## When to graduate to the app flow

- Need exact reproducibility (e.g. you're shipping a model server) → migrate inference to a separate app project (Layout 02) with `pyproject.toml` + `uv.lock` (`references/3-app/backend/app-skeleton.md`).
