...
Owns three things for a backend app: **the run-service-vs-distributable layout rule** (flat `app/` vs `src/<pkg>/`), **the `pyproject.toml` + `uv.lock` + `uv sync` flow** (decision + mechanics), and **the top-level skeleton** inside `app/` (`main.py`, `core/`, feature folders). For app projects — Layout 01–03, 05. ML projects use a different flow (`references/3-app/backend/ml-python-flow.md`). Domain *grouping* (T2) and feature-folder *internals* are owned below this file — links in place.
...
**Default for a backend is the run-service column.** Only reach for src-layout when the thing is genuinely distributable. (A distributable's *repo shape* and publishing mechanics are L2 — `references/2-repo/layouts/06_embeddable-package.md`.)
...
# dev uses `uvicorn --reload`; prod uses gunicorn — worker model owned by references/3-app/backend/serving.md
...
- `references/3-app/backend/domain-grouping.md` — feature → domain grouping (T2), domain naming
- `references/4-feature/feature-folders.md` — the `{router,service,repository,models}.py` contract
- `references/3-app/backend/migrations.md` — Alembic vs raw-SQL and the migration owner
- `references/3-app/backend/serving.md` — production worker model (gunicorn CMD)
- `references/3-app/backend/ml-python-flow.md` — the ML flow this is deliberately different from
