...
- The raw-SQL three-file pattern (`references/3-app/backend/raw-sql-recipe.md`) fits naturally here: the `.sql` files are readable by every consumer regardless of language.
...
- **Scope note:** for the **single-backend** case, the existing default stands — that backend owns its migrations, entrypoint-migrates-on-deploy is fine (`references/3-app/backend/migrations.md`). The neutral owner is the two-backend escalation.
...
- The two frontends pair naturally with the **plane-grouped `apps/` topology** (`apps/client/{platform,admin}` — see the grouping variants in `references/2-repo/grouping-topology.md`).
- Backend secrets never enter either frontend's env scope (`references/2-repo/env-and-config/frontend-env-isolation.md`) — doubly load-bearing here, since the admin plane's URLs/keys must not leak into the public bundle.
- Exposure follows the split: the admin frontend + `api-admin` can bind to an internal expose tier while the platform pair is public — see the expose tiers in `references/2-repo/runtime/docker-overview.md` and route separation in `references/2-repo/deployment/proxy-and-exposure.md`.
...
- `references/3-app/backend/migrations.md` — migration style + DDL owner decision that `apps/db` implements
- `references/3-app/backend/raw-sql-recipe.md` — the raw-SQL three-file mechanics `apps/db` can run
- `references/4-feature/types-and-contracts.md` — DTO ownership rules both backends follow
- `references/2-repo/grouping-topology.md` — the grouping topology this pairs with
- `references/2-repo/env-and-config/frontend-env-isolation.md` — env split + frontend isolation
- `references/2-repo/00_charter.md` — the repo-level charter that routes here
