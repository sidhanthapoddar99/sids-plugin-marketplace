# Postgres usage conventions

How to run Postgres well once it's the chosen engine: image, compose service block, init scripts, connection, healthcheck, backup, extensions. **Whether** to pick Postgres (vs SQLite) is a choice owned by `references/2-repo/databases-provisioning.md`.
...
State bind-mounts under `data/`, config under `infra/` — the placement doctrine is owned by `references/2-repo/databases-provisioning.md`.
...
`postgres` is the compose service name; inside compose's internal network it resolves automatically. (Single-stack advice: on a network **shared across stacks**, generic names collide — project-prefix them, e.g. `myapp-postgres`; see `references/2-repo/runtime/multi-stack.md`.)
...
Reach for a Postgres extension before pulling in a separate engine — vector (`pgvector`), time-series (TimescaleDB), fuzzy search (`pg_trgm`) all live here. The cross-engine selection table is in `references/2-repo/databases-provisioning.md`.

## Anti-patterns

- Running `Base.metadata.create_all()` in production — migrations should be explicit (see `references/3-app/backend/migrations.md`)
...
- `references/2-repo/databases-provisioning.md` — engine choice (SQLite vs Postgres) + infra/ vs data/ placement
- `references/3-app/backend/migrations.md` — Alembic vs raw-SQL decision
- `references/3-app/database-usage/redis.md` — cache/session companion
- `references/2-repo/runtime/docker-details.md` — bind-mount discipline
