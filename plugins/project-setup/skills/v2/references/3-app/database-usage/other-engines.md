...
How to run the less-common engines once chosen: MongoDB, Neo4j, Kuzu, SeaweedFS, Meilisearch — compose blocks, config snippets, and per-engine fit notes. **Which** engine to pick (the cross-engine selection table + right-floor rule) is a choice owned by `references/2-repo/databases-provisioning.md`.
...
4. **Internal network** unless host port exposure is needed via the `expose` modifier of `ctl up` (see `references/2-repo/deployment/proxy-and-exposure.md`).

State bind-mounts under `data/<service>/` (gitignored), config files under `infra/<service>/` (committed) — this placement doctrine is owned by `references/2-repo/databases-provisioning.md`.
...
- `references/2-repo/databases-provisioning.md` — engine choice (right floor) + infra/ vs data/ placement
- `references/3-app/database-usage/postgres.md` — the default relational engine + its extensions
- `references/3-app/database-usage/redis.md`
