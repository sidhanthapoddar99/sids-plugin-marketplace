...
State bind-mounts under `data/`, config under `infra/` — the placement doctrine is owned by `references/3-app/04-database/00_provisioning.md`.
...
`postgres` is the compose service name; inside compose's internal network it resolves automatically. (Single-stack advice: on a network **shared across stacks**, generic names collide — project-prefix them, e.g. `myapp-postgres`; see `references/2-repo/04-docker/03_multi-stack.md`.)
...
Reach for a Postgres extension before pulling in a separate engine — vector (`pgvector`), time-series (TimescaleDB), fuzzy search (`pg_trgm`) all live here. The cross-engine selection table is in `references/3-app/04-database/00_provisioning.md`.

## Anti-patterns

- Running `Base.metadata.create_all()` in production — migrations should be explicit (see `references/3-app/04-database/01_migrations.md`)
...
- `references/3-app/04-database/00_provisioning.md` — engine choice (SQLite vs Postgres) + infra/ vs data/ placement
- `references/3-app/04-database/01_migrations.md` — Alembic vs raw-SQL decision
- `references/3-app/04-database/06_redis.md` — cache/session companion
