...
Setup and daily mechanics for the Alembic migration style. Zero decisions here — **which** style to use (Alembic vs raw-SQL vs native tool vs none) and **which** run model (entrypoint vs one-shot vs neutral owner) are owned by `references/3-app/04-database/01_migrations.md`. This file is the concrete "how".
...
```bash
# from the backend dir (flat layout — see references/3-app/02-backend/00_app-skeleton.md)
...
- **`config.set_main_option("sqlalchemy.url", settings.db_url)`** — the URL comes from config (which reads root `.env` via `${VAR}` — see `references/2-repo/03-env-config/01_per-service-config.md`), so there's exactly one source of truth and no secret in `alembic.ini`.
...
from app.db import Base    # your SQLAlchemy declarative base
...
Always review and edit the output before committing. When to reach for autogenerate vs hand-write is owned by `references/3-app/04-database/01_migrations.md`.

## Docker entrypoint — the container migrates itself

The mechanics of the **entrypoint-migrates** run model (the single-replica default; run-model choice owned by `references/3-app/04-database/01_migrations.md`). The backend container's entrypoint runs `alembic upgrade head` on boot, before launching the server — first run and every subsequent boot (idempotent: a no-op if already at head).
...
For the multi-replica (one-shot) and two-backend (neutral `apps/db`) variants, see the run-model table in `references/3-app/04-database/01_migrations.md`.
...
- Review-before-commit and the autogen/hand-write split rules are owned by `references/3-app/04-database/01_migrations.md`.

## See also

- `references/3-app/04-database/01_migrations.md` — the style + run-model decisions this recipe implements
- `references/3-app/04-database/03_raw-sql-recipe.md` — the raw-SQL variant's mechanics
- `references/3-app/02-backend/00_app-skeleton.md` — the flat `app/` layout and `pyproject` + `uv` flow
- `references/2-repo/03-env-config/01_per-service-config.md` — the `config.yaml` + `${VAR}` the URL loader reads
- `references/3-app/04-database/04_sqlite.md` — why `render_as_batch=True` is required for SQLite
