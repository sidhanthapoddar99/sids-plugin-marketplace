...
| **Type / DTO ownership** | `models.py` = the feature's API contract; frontend types inferred from zod in `api/`; cross-app entities in `packages/types`; no cross-domain DTO imports; no shared models package between backends. | `references/4-feature/03_types-and-contracts.md` |
