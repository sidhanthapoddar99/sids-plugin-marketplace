...
|---|---|---|
| Repo layout | Layout 02, **plane-grouped** topology (T1 tripped) | `references/2-repo/01-layouts/00_grouping-topology.md` |
| Backend count | two — separate security postures (admin vs user) | `references/3-app/02-backend/02_two-plane-split.md` |
| DDL owner | neutral `apps/db`, run only via `ctl migrate` | `references/3-app/02-backend/02_two-plane-split.md`, `references/3-app/04-database/01_migrations.md` |
| Workspace rooting | polyglot repo → JS workspace roots at `apps/client/`; repo root manifest-free | `references/2-repo/01-layouts/00_grouping-topology.md`, `references/2-repo/02-root-hygiene/00_root-and-hygiene.md` |
| Package placement | frontend-only packages inside the client group | `references/2-repo/01-layouts/00_grouping-topology.md` |
| Backend internals | domain layer inside each backend (T2) | `references/3-app/02-backend/01_domain-grouping.md` |
| Frontend internals | the hard `src/` skeleton inside each frontend | `references/3-app/03-web-app/00_app-skeleton.md` |
| Migration mechanics | raw-SQL three-file pattern (language-neutral) | `references/3-app/04-database/03_raw-sql-recipe.md` |
...
|---|---|---|---|
| Repo layout | Layout 02, plane-grouped | T1 (tripped: 2 frontends + 2 backends) | references/2-repo/01-layouts/00_grouping-topology.md |
| Identity planes | split: api-platform (users) + api-admin (operators) | — | references/3-app/02-backend/02_two-plane-split.md |
| Database | one Postgres; per-table single writer; no shared models pkg | — | references/3-app/02-backend/02_two-plane-split.md |
| DDL owner | apps/db (neutral); migrate only via `ctl migrate` | — | references/3-app/04-database/01_migrations.md |
| Migration style | raw-SQL three-file (language-neutral consumers) | — | references/3-app/04-database/03_raw-sql-recipe.md |
| Workspace root | apps/client/ (polyglot repo; repo root manifest-free) | T10 (watched) | references/2-repo/01-layouts/00_grouping-topology.md |
| Frontend packages | ui/styles/types/services inside apps/client/packages/ | — | references/2-repo/01-layouts/00_grouping-topology.md |
| Backend↔frontend | core backend (domain-first APIs) | — | references/2-repo/01-layouts/02_multi-app-monorepo.md |
| Domain layers | api-platform: catalog/orders/access; api-admin: moderation/operators | T2 (tripped in api-platform) | references/3-app/02-backend/01_domain-grouping.md |
...
```
marketplace/
├── .env                              # shared backend/infra vars only, gitignored — 00_env-precedence.md
├── .env.example                      # the committed contract
├── .env.production                   # optional, compose env_file for prod
├── .mise.toml                        # runtime contract: python + node + uv toolchains — 06-runtime-environment/01_mise.md
...
│   │   │   ├── config.yaml           # reads root .env via ${VAR} — 03-env-config/01_per-service-config.md
...
│   │   │   ├── tests/
│   │   │   ├── Dockerfile
│   │   │   └── README.md             # this backend's host dev loop — 01_readme-three-paths.md
...
│       ├── turbo.json                # globalEnv lists every VITE_* cache-buster — references/3-app/01-structure-and-stack/02_workspaces-mechanics.md
│       ├── tsconfig.json             # base TS config extended by each app
│       ├── platform/                 # PUBLIC frontend (buyers/sellers)
│       │   ├── package.json + .env   # VITE_* only — 03-env-config/02_frontend-env-isolation.md
│       │   ├── vite.config.ts        # proxies /api/* to api-platform in dev — 04-docker/04_proxy-and-exposure.md
│       │   ├── tailwind.config.ts    # extends packages/tailwind-config
│       │   ├── index.html            # data-theme="light" — references/3-app/05-package/01_tokens-setup.md
│       │   └── src/                  # the hard skeleton — references/3-app/03-web-app/00_app-skeleton.md
│       │       ├── main.tsx  App.tsx
...
│           └── typescript-config/
├── infra/                            # CONFIG only, not data — references/3-app/04-database/00_provisioning.md
│   ├── nginx/
...
├── README.md                        # root README: three paths — 01_readme-three-paths.md
└── LICENSE
```
...
This is a polyglot repo (Python backends + TS frontends), so the JS workspace roots at the **frontend group folder** `apps/client/`, not the repo root: `package.json`, `pnpm-workspace.yaml`, lockfile, and `node_modules/` all live there and the repo root stays manifest-free. `ctl` bridges back — `ctl dev` / `ctl build` `cd` into `apps/client/` and delegate to turbo. Git hooks use **lefthook at the repo root** (husky expects the git root; this repo's workspace isn't there). Governed by `references/2-repo/01-layouts/00_grouping-topology.md` (rooting + frictions), `references/2-repo/02-root-hygiene/00_root-and-hygiene.md` (manifest-free root, T10).

## Which references govern each part

| Part of the tree | Governing reference |
|---|---|
| Whole layout + when plane-grouping is right (T1) | `references/2-repo/01-layouts/00_grouping-topology.md` |
| `apps/server/{api-platform,api-admin}` split + `apps/db` | `references/3-app/02-backend/02_two-plane-split.md` |
| Migration style + DDL owner decision | `references/3-app/04-database/01_migrations.md` |
...
| `ctl` dispatcher model + commands | `references/2-repo/05-ctl-scripts-tooling/00_script-overview.md`, `.../01_script-usage.md` |
| `nginx/` route separation (public vs internal edge) | `references/2-repo/04-docker/04_proxy-and-exposure.md` |
| Production serving (workers, migrations-on-deploy) | `references/3-app/10-deployment/00_serving.md`, `references/2-repo/04-docker/05_production-readiness.md` |
| `.mise.toml` runtime contract | `references/2-repo/06-runtime-environment/01_mise.md` |
| Root README + per-service READMEs | `references/2-repo/02-root-hygiene/01_readme-three-paths.md` |
| `infra/` vs `data/`, engine choice | `references/3-app/04-database/00_provisioning.md` |
...
- `references/2-repo/01-layouts/02_multi-app-monorepo.md` — the layout, full scaling spectrum, core-vs-BFF axis
- `references/2-repo/01-layouts/00_grouping-topology.md` — flat/plane-grouped/hybrid decision (T1), rooting, package scope
- `references/3-app/02-backend/02_two-plane-split.md` — the admin/user split decision this example instantiates
