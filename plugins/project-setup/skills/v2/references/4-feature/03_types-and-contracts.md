...
- API types are **inferred, not authored** — the zod schema is the source of truth; the parse boundary that produces it is owned by `references/4-feature/02_api-and-pages.md`. A hand-written response type beside a schema is a twin that will drift.
...
- `references/4-feature/02_api-and-pages.md` — the zod parse boundary and api layer that produce the inferred frontend types
- `references/4-feature/01_feature-folders.md` — the `{router,service,repository,models}.py` feature shape `models.py` DTOs sit in
