...
- **Every contract has one owner** — a DTO, an exported type, a canonical shape each live in exactly one file (see `references/4-feature/types-and-contracts.md`).
...
```bash
# server communication outside the api layer (frontend) — rule owned by api-and-pages.md
...
#   — the four commands owned by styling-discipline.md § grep-enforceable
