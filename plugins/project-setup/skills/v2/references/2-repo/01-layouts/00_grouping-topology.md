...
This is a decision file. The multi-frontend **mechanics** (pnpm/turbo config bodies, catalog, `globalEnv`, `ctl` shape) live in `references/3-app/01-structure-and-stack/02_workspaces-mechanics.md`; package **internals** (what's inside `ui`/`styles`/`types`) in `references/3-app/05-package/00_shared-packages.md`.
...
└── client/
    ...
    ├── pnpm-workspace.yaml   #   (repo root stays manifest-free — 00_root-and-hygiene.md)
...
The plane-grouped topology pairs naturally with the admin/user **two-plane split** (`references/3-app/02-backend/02_two-plane-split.md`) and forces polyglot **workspace rooting** (next section).
...
Even a JS-only repo's root `package.json` stays orchestration-only — zero runtime deps, no source (`references/2-repo/02-root-hygiene/00_root-and-hygiene.md` § root manifest rule).

In the polyglot case, `ctl` is the bridge back to the root: `ctl dev` / `ctl build` cd into the workspace root and delegate — mechanics owned by `references/3-app/01-structure-and-stack/02_workspaces-mechanics.md` § ctl shape. Workspace globs are relative to the workspace root, so the polyglot variant anchors them at the group folder (and the root-rooted variant must exclude non-JS apps); config bodies owned by `references/3-app/01-structure-and-stack/02_workspaces-mechanics.md`.
...
|---|---|---|
| husky / git hooks | expects to install at the git root | use lefthook at the repo root instead (`references/2-repo/05-ctl-scripts-tooling/04_lefthook.md`) — its commands can `cd` into the workspace |
...
Package internals (export surface, `~15`-component grouping, tailwind-config wiring) are owned by `references/3-app/05-package/00_shared-packages.md`.
...
- Polyglot repo with the JS workspace rooted at the repo root = finding — propose group-folder rooting (`references/2-repo/02-root-hygiene/00_root-and-hygiene.md`).
...
- `references/2-repo/00_index.md` — the L2 charter this decision serves
- `references/2-repo/01-layouts/02_multi-app-monorepo.md` — the multi-app shape that consumes this topology
- `references/2-repo/02-root-hygiene/00_root-and-hygiene.md` — root-as-index, orchestration-only root manifest (rooting links back here)
- `references/3-app/01-structure-and-stack/02_workspaces-mechanics.md` — pnpm/turbo/bun config bodies, catalog, `globalEnv`, `ctl` shape
- `references/3-app/05-package/00_shared-packages.md` — what lives inside `ui`/`styles`/`types` and the export surface
- `references/3-app/02-backend/02_two-plane-split.md` — the admin/user split that pairs with plane-grouped
