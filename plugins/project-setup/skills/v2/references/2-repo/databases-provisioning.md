...
- Alembic with SQLite needs `render_as_batch=True` for `ALTER`s (see `references/3-app/backend/alembic-recipe.md`)
...
- Prod: `/srv/<app>/data` (set in `.env.production`)
- Test: `/tmp/<app>-test-${RUN_ID}` (set in CI)
