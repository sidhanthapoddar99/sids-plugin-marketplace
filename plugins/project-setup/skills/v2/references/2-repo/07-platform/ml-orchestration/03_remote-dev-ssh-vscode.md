...
`scripts/cloud/remote-dev.sh` (a thin wrapper over the provider CLI — `references/2-repo/ml-orchestration/custom-orchestrator.md`) must do all of this in one command:
...
**Default**: option B for cost (no GPU cost while idle); option A when the agent itself needs heavy file ops or GPU-direct work. Agent-side mechanics: `references/2-repo/ml-orchestration/agent-ssh-access.md`.
