...
`~/.ssh/config` entry (written by `scripts/cloud/remote-dev.sh` or by hand):
...
- The teardown command if the agent needs to stop the box (`./scripts/cloud/teardown.sh my-ml-dev`)
...
(declared in `~/.ssh/config`, provisioned via `scripts/cloud/remote-dev.sh`).

When you need to run training / eval / inference:

1. Spin up if not running: `ctl cloud up`
2. Check status: `./scripts/cloud/status.sh my-ml-dev` or `ssh my-ml-dev 'nvidia-smi'`
...
5. Stop when done: `./scripts/cloud/teardown.sh my-ml-dev`
...
- Submit jobs via the `scripts/cloud/` wrappers — yes
...
```json
{
  "permissions": {
    "allow": [
      "Bash(ssh my-ml-dev:*)",
      "Bash(rsync:*)",
      "Bash(./scripts/cloud/train-spot.sh:*)",
      "Bash(./scripts/cloud/status.sh:*)",
      "Bash(./scripts/cloud/wait-for-run.sh:*)"
    ],
    "ask": [
      "Bash(./scripts/cloud/remote-dev.sh:*)",
...
Install Claude Code in the provisioning script's setup step:

```bash
# scripts/cloud/remote-dev.sh — setup phase, runs on the box
...
- Hardcoded IP in `~/.ssh/config` for an ephemeral box — the provisioning script rewrites the entry on every bring-up
- Skipping `ControlMaster` — every command pays the handshake
- Giving the agent the teardown script without "ask" — surprise teardowns
