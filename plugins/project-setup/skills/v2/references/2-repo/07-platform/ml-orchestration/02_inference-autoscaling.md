...
The controller is itself **on-demand** (no preemption). It watches the spot replicas, and when one disappears it kicks off a new one and registers it with the LB. It can be a small script on a cheap VM or the custom orchestrator CLI (`references/2-repo/ml-orchestration/custom-orchestrator.md`).
