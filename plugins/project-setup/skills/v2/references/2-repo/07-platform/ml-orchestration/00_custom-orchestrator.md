...
```
scripts/cloud/
├── remote-dev.sh          # one-command GPU dev box (see remote-dev-ssh-vscode.md)
...
├── serve.sh               # optional inference service (see inference-autoscaling.md)
...
- Each wrapper is **thin** — provider CLI calls + ssh, no business logic. The same thin-wrapper principle as `ctl` (`references/2-repo/runtime/script-overview.md`); `ctl cloud <verb>` may route to these.
...
- Spot re-acquisition + checkpoint mounting live in the run wrappers; the contract is owned by `references/2-repo/ml-orchestration/spot-instances-and-checkpoints.md`.
...
3. **Spot policy** — spot for training/sweeps, on-demand for inference SLAs and final runs (`spot-instances-and-checkpoints.md`).
