# Custom orchestrator — placeholder for a future bespoke tool

If dstack and SkyPilot don't cover a use case, the escape hatch is to build a thin custom orchestrator. Pattern is similar to a Layout 05 Go CLI: a small binary that calls cloud SDKs / kubectl / SSH, with structured state across runs.
...
For now, this file is a **placeholder**. If one actually gets built, the conventions land here:
...
4. **OK, then** — describe the pattern (a thin Go CLI, Layout 05-style), point at this file as a stub, and proceed
