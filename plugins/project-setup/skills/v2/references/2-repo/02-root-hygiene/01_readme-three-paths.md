...
3. **Manual (without `ctl`)** — the same stack run *without* the dispatcher: raw `docker compose -f docker/…`, and per-service host runs that **defer to each service's own README** (ctl-agnostic — see "Two README levels" below).

These map onto the three startup paths the audit enforces — `ctl` (dev + up), raw compose, and no-docker host run — grouped as "with ctl" (quick start) vs "without ctl" (manual).
...
Full design docs in `docs/` (agent-knowledge-system, scaffolded via `/agent-ks-init`); see the `docs/` README / the agent-ks skill for running the site locally.
