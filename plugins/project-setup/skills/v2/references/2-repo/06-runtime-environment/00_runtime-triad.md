...
If any path is broken, the repo has invisible debt — `/ps-setup audit` checks for all three (full README contract: `references/2-repo/readme-three-paths.md`). Complex (Layout 05) setups add a fourth: building the orchestrator binary (`references/2-repo/runtime/complex-setups.md`).
