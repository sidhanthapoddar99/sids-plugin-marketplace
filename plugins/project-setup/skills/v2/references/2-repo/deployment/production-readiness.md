...
- **Single replica** (dev, single-VM prod, SQLite / one Postgres): the backend **container's entrypoint runs `alembic upgrade head` on boot**, then launches the server. Simple, idempotent, no separate step. See `references/3-app/backend/alembic-recipe.md` § "Migrations in Docker".
