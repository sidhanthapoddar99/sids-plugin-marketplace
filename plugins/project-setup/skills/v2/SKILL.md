...
13. **Nesting follows service count:** one service total → top-level `./<name>/`; more than one → group under `apps/<name>/`. One-liner: **flat for run-services, `src/` for frontends and packages, nothing loose in root.** Owner: `references/2-repo/grouping-topology.md`.
...
- `references/2-repo/ml-orchestration/` — start at `overview.md`; dstack work also composes with the sibling `dstack` plugin's skill.
