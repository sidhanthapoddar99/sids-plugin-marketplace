...
- **Compose lives in `docker/`**, split on two axes: **profiles** (which services run — data core has no profile and is always up; apps opt in via `profiles: [app]`/`[edge]`) and **`--config` overlays** (`compose.yaml` base + `compose.{expose,traefik,prod}.yaml` for how they run). Base is port-less; profiles do ~90% of the work because dev runs on the host.
- **One control dispatcher at repo root** — `ctl` — is the single entrypoint: `ctl dev` (local host loop, hot reload, auto-starts the data core) / `ctl up [profile…] [--config=name…]` (containers — profiles select services, configs overlay how they run; production is `ctl up app edge --config=prod`, no separate `prod` verb) / `down`·`ps`·`logs` / `status`·`setup`·`migrate`. It delegates to `docker compose`, a process runner (`process-compose`/`mprocs`), and `scripts/*.sh`; callable bare via mise PATH.
