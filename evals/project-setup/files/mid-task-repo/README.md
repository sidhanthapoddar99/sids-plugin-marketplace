...
| Area | Pick |
|---|---|
| Backend | Python `3.13` / FastAPI · Rust `3.13` / Axum · Go `3.13` (CLI) |
...
## Lock files
The template ships no `bun.lock`, `uv.lock` or `Cargo.lock`: every dependency is a `3.13` placeholder. `ctl setup` installs and creates them. Commit them.

## Layout
```
apps/      api example-engine-rust example-multi-web-app/{landing,app,docs}
           web example-dashboard-nextjs example-tui-go packages database
